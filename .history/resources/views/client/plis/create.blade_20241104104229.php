    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            ...
            <form action="{{ url('client/add-pli') }}" method="POST" class="mt-5 bg-light p-5 rounded shadow" enctype="multipart/form-data" id="pliForm">
                ...
                <!-- Sélection du nombre de références -->
                <div class="mb-3">
                    <label for="reference" class="form-label">Nombre de références<span class="text-danger">*</span></label>
                    <input type="number" class="form-control bg-primary text-white @error('reference') is-invalid @enderror" id="reference" name="reference" required oninput="generateReferenceFields()">
                    @error('reference')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                <!-- Conteneur pour les champs de saisie de références -->
                <div id="referenceFieldsContainer"></div>
                <div class="mb-3">
...
<script>
    ...
    function generateReferenceFields() {
        const referenceCount = parseInt(document.getElementById('reference').value) || 0;
        const container = document.getElementById('referenceFieldsContainer');
        
        // Vider le conteneur avant de générer les nouveaux champs
        container.innerHTML = '';

        // Créer les champs de saisie de références en fonction de la valeur entrée
        for (let i = 1; i <= referenceCount; i++) {
            const inputField = document.createElement('div');
            inputField.classList.add('mb-3');
            inputField.innerHTML = `
                <label for="reference_${i}" class="form-label">Référence ${i}</label>
                <input type="text" class="form-control" id="reference_${i}" name="reference_${i}" required>
            `;
