...
use App\Models\Pli;
use App\Models\Reference; // Importez le modèle Reference
use Carbon\Carbon;
...
class PliController extends Controller
{
    ...
    public function store(PliFormRequest $request)
    {
        ...
        // Enregistrer les références dans la table Reference
        if ($request->has('reference')) {
            foreach ($request->input('reference') as $referenceValue) {
                Reference::create([
                    'pli_id' => $pli->id, // Associez la référence au pli
