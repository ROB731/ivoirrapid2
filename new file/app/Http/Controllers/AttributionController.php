...
public function massAttribuer(Request $request)
{
    $pliIds = $request->input('pli_ids');
    $attributions = $request->input('attributions');

    if (!$pliIds) {
        return redirect()->back()->with('error', 'Veuillez sélectionner au moins un pli.');
    }
    ...
    try {
        ...
        foreach ($pliIds as $pliId) {
            $pli = Pli::find($pliId);
            if ($pli) {
                $pli->attribution()->create([
                    'type' => $attributions[$pliId],
                    'coursier_id' => auth()->user()->id,
                ]);
            }
        }

        DB::commit(); // Validation si tout est bon
        return redirect()->back()->with('success', 'Attributions effectuées avec succès !');
    } catch (\Exception $e) {
        DB::rollback(); // Annulation en cas d'erreur
        return redirect()->back()->with('error', 'Erreur lors de l’attribution : ' . $e->getMessage());
    }
}
